...
//		ALPHAVANTAGE API KEY = VNEY4VV2AWF1EB51
		try {
			...
			if (responsecode == 200) {
				...
				for (int i = 0; i < size; i++) {
					JsonObject object = jsonArray.get(i).getAsJsonObject();
					name = object.get("symbol").getAsString();
					id = object.get("id").getAsString();
	...
	public ArrayList<String> getNames(ArrayList<String> coinSymbols) {
		ArrayList<String> coinNames = new ArrayList<String>();
		for(int i = 0; i < coinSymbols.size(); i++) {
			coinNames.add(getCryptoID(coinSymbols.get(i)));
		}
		return coinNames;
	}
	...
	public static void main(String[] args) {
		...
		System.out.println(acl.getCryptoID("btc"));
	}
