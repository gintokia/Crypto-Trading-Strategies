/**
 * The DataVisualizationCreator is responsible for rendering table and bar graph with updated data, and onto the UI
 * 
 * @author Abdul Khan
 */
...
public class DataVisualizationCreator {
	...
	/**
	 * Creates table and bar graph with updated information
	 * 
	 * @param dataObject: used to create table
	 * @param dataSets: used to create bar graph
	 */
	public void createCharts(ArrayList<TradeResult> dataObject, ArrayList<DataSet> dataSets) {
//		createTextualOutput();
		createTableOutput(dataObject);
//		createTimeSeries();
//		createScatter();
		createBar(dataSets);
	}
...
//		MainUI.getInstance().updateStats(scrollPane);
	...
	/**
	 * Creates table by retrieving information from given data object
	 * 
	 * @param dataObject: contains information under the column names
	 */
	private void createTableOutput(ArrayList<TradeResult> dataObject) {
		// Dummy dates for demo purposes. These should come from selection menu
		Object[] columnNames = {"Trader","Strategy","CryptoCoin","Action","Quantity","Price","Date"};
		
		Object[][] data = new Object[dataObject.size()][7];
		
		for(int i = 0; i < dataObject.size(); i++) {
			data[i][0] = dataObject.get(i).getTradingBrokerName();
			data[i][1] = dataObject.get(i).getTradingStrategyName();
			data[i][2] = dataObject.get(i).getCoinName();
			data[i][3] = dataObject.get(i).getAction();
			data[i][4] = dataObject.get(i).getQuantity();
			data[i][5] = dataObject.get(i).getPrice();
			data[i][6] = dataObject.get(i).getDate();
		}
	...
	/**
	 * Creates a bar graph with data that is retrieved from the list of data sets given
	 * 
	 * @param dataSets: contains information that is to be rendered onto the bar graph
	 */
	private void createBar(ArrayList<DataSet> dataSets) {
		
		DefaultCategoryDataset dataset = new DefaultCategoryDataset();
		
		for(int i = 0; i < dataSets.size(); i++) {
			dataset.setValue(dataSets.get(i).getNumberOfTrades(), 
					dataSets.get(i).getTradingBrokerName(),
					dataSets.get(i).getTradingStrategyName());
		}
		...
		rangeAxis.setRange(new Range(1, 20));
